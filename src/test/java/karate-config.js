function fn() {
    var env = karate.env; // get system property 'karate.env'
    karate.log('karate.env system property was:', env);

    if (!env) {
        env = 'dev';
    }

    // karate.port se inyecta desde PruebaTest cuando el servidor arranca en
    // RANDOM_PORT; el fallback a 8080 sirve para ejecución manual con servidor
    // ya levantado en localhost.
    var port = karate.properties['karate.port'] || '8080';

    var config = {
        env: env,
        myVarName: 'someValue',
        baseUrl: 'http://localhost:' + port
    }

    /**
     * Drivers for tests - currently configured value is good for Linux
     */
    karate.configure('driver', {
        type: 'chrome',
        // descomentar para chromium bajo linux
        // executable: '/usr/bin/chromium-browser',
        executable: '/usr/bin/google-chrome',
        //executable: "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
        addOptions: ["--remote-allow-origins=*", "--incognito"],
        showDriverLog: true
    })

    if (env == 'dev') {
        // customize
        // e.g. config.foo = 'bar';
    } else if (env == 'e2e') {
        // customize
    }
    return config;
}
